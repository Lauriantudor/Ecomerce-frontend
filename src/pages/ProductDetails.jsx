...
function ProductDetails() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Feedback vizual instant de succes
      toast.success("Produs adăugat", {
        description: `Am adăugat ${quantity} × ${product.name} în coșul tău. 🛍️`,
      });

      // Resetăm selectorul înapoi la 1 după adăugarea cu succes
      setQuantity(1);
    } catch (err) {
      console.error("Eroare la adăugarea în coș:", err);
      toast.error("Eroare la adăugare", {
        description: "Nu s-a putut adăuga produsul în coș. Încearcă din nou.",
      });
    } finally {
      setIsAdding(false);
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 bg-white dark:bg-zinc-900 border border-stone-200/60 dark:border-zinc-800 rounded-3xl p-6 lg:p-8 shadow-sm dark:shadow-2xl items-start transition-colors duration-300">
        ...
        <div className="flex flex-col justify-between space-y-6 md:pt-4 h-full">
          <div>
            ...
            <hr
              className="border-stone-200/60 dark:border-zinc-800 my-6"
              aria-hidden="true"
            />
